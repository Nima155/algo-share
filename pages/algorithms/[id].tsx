...
import { IAlgorithm, IComment, IExpandedComment } from '../../utils/types'
...
export default function Algorithm({
	...
	algorithmInfo: IAlgorithm
	comments: IExpandedComment[]
}) {
	const { algorithm, author, code, language, description } = algorithmInfo
			...
			>
				...
				<div className="bg-gray-100 text-black p-2 flex flex-col flex-grow rounded-b gap-1">
					...
					{/* TODO: Implement comment system! 
						...
						*/}
					<CommentSection algoId={query.id || ''} comments={comments} />
				</div>
			</GeneralContainer>
		</Layout>
	)
...
export async function getStaticProps({ params }: { params: { id: string } }) {
	...
	const { data } = await getAssociatedComments(params.id)

	return {
		props: {
			algorithmInfo: allInformationOnAlgorithm,
			comments: data,
		},
		revalidate: 60,
	}
